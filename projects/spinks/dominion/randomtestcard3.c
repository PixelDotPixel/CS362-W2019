...
int main()
{
    int cardTested = village;
    int result = 0;
    int tests = 0;
    int handOrDeck, emptyOrFull;
    int testSeed = 0;
    int Player = 0;
    int numPlayers = 2;
    int i, j;
    ...
    for(numPlayers = 2; numPlayers <= 4; numPlayers++){
        for(testSeed = 1; testSeed < 32000; testSeed++){
            ...
            emptyOrFull = (rand() + testSeed) % 2;                      //Random empty or full deck
            ...
            if(emptyOrFull == 1 && handOrDeck == 0){
                test.discardCount[Player] = base.deckCount[Player];
                for(i = 0; i < test.deckCount[Player]; i++){
                    test.discard[Player][i] = test.deck[Player][i];
                }
            ...
            if(handOrDeck == 0){
                ...
                cardEffect(cardTested, randChce1, randChce2, randChce3, &test, randLoc, NULL);

                //Assert that the card was played/discarded
                // and that the hand has gained 1 card, and lost 1 card 
                // and that the discard pile has gained 1 card
                // and the action points have increased by 2
                // and that all other states are unchanged
                result += assertC(test.handCount[Player], base.handCount[Player], "test.handCount[Player]", "base.handCount[Player]");
                result += assertC(test.deckCount[Player], base.deckCount[Player] - 1, "test.deckCount[Player]", "base.deckCount[Player] - 1");
                result += assertC(test.discardCount[Player], base.discardCount[Player] + 1, "test.discardCount[Player]", "base.discardCount[Player]");
                result += assertC(test.numActions, base.numActions + 2, "test.numActions", "base.numActions");
                ...
                cardEffect(cardTested, randChce1, randChce2, randChce3, &test, j, NULL);
                //Assert that the card was played/discarded
                // and that the hand has gained n cards, and lost 1 card 
                // and that the discard pile has gained 1 card
                // and the action points have increased by 2
                // and that all other states are unchanged
                result += assertC(test.handCount[Player], base.handCount[Player] + drawnIndex, "test.handCount[Player]", "base.handCount[Player] + drawnIndex");
                result += assertC(test.deckCount[Player], base.deckCount[Player] - 1 - drawnIndex,"test.deckCount[Player]", " base.deckCount[Player] - 1 - drawnIndex");
                result += assertC(test.discardCount[Player], base.discardCount[Player] + 1,"test.discardCount[Player]", "base.discardCount[Player]");
                result += assertC(test.numActions, base.numActions + 2, "test.numActions", "base.numActions");
                result += assertC(test.numBuys, base.numBuys, "test.numBuys", "base.numBuys");
